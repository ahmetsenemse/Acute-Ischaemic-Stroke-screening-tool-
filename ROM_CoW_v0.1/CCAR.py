...
for l in range(0,L): 
    ...
    for k in range(0,int(Ql)):
        ...
        phi_k_t  = clf_time[counter].predict(v_dt)
...
with open('clf_time6.pkl','wb') as f:
    pickle.dump(clf_time,f)
with open('clf_param6.pkl','wb') as f:
    pickle.dump(clf_param,f)
np.save("Pl_Sig6",Pl_Sig)
np.save("Ql_list6",Ql_list)
np.save("L6",L)
np.save("V6",V)
"""
S = np.zeros((N_h,N_t))
...
ax11.plot(v_dt,v_urb[0,:],'r-',linewidth=1, markersize=0.5, label='Predicted velocity Vessel1')
ax11.plot(v_dt,S[0,:],'b-',linewidth=1, markersize=0.5, label='Reference velocity Vessel1')
