...
for train_size in [400]:
    ...
    for j in range(0,N_mu):    
        for i in MCALvessels:
            ...
            if os.path.isfile(name2) and os.path.isfile(name1):
                ...
                if i==50 or i==51 or i==52 or i==53 or i==58 or i==60 or i==59 or i==62:
                    target=np.array([1,0])
                else :
                    target=np.array([0,1])
    ...
    for j in range(0,N_mu):    
        for i in MCARvessels:
            ...
            if os.path.isfile(name2) and os.path.isfile(name1):
                ...
                if i==68 or i==69 or i==70 or i==71 or i==76 or i==77 or i==78 or i==80:
                    target=np.array([1,0])
                else :
                    target=np.array([0,1])
    ...
    name='model_segment_MCA_full_'+str(train_size)+'.h5'
    model.save(name)
    ...
    name='fpr_MCA_full_'+str(train_size)+'_'
    np.save(name,fpr)
    
    name='tpr_MCA_full_'+str(train_size)+'_'
    np.save(name,tpr)
    
    name='cma_MCA_full_'+str(train_size)+'_'
    np.save(name,cma)
    
    name='f1test_MCA_full_'+str(train_size)+'_'
    np.save(name,F1_test)
    
    name='P_nor_MCA_full_'+str(train_size)+'_'
    np.save(name,A_mean)
    
    name='P_std_MCA_full_'+str(train_size)+'_'
    np.save(name,A_std)
    
    name='V_nor_MCA_full_'+str(train_size)+'_'
    np.save(name,V_mean)
    
    name='V_std_MCA_full_'+str(train_size)+'_'
    np.save(name,V_std)
    
    name='Age_nor_MCA_full_'+str(train_size)+'_'
    np.save(name,Age_mean)
    
    name='Age_std_MCA_full_'+str(train_size)+'_'
