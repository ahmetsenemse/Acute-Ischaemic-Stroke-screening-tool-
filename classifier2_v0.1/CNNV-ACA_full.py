...
for train_size in [1000]:
    ...
    for j in range(0,N_mu):    
        for i in ACALvessels:
            ...
            name2="..\samples\CoW_"+str(j)+"_ACAL"+str(int(i))+"0.csv"
            ...
            if os.path.isfile(name2) and os.path.isfile(name1):
                ...
                if i==86 or i==87 or i==88 or i==89 or i==90 or i==94 or i==95 or i==96 or i==97 or i==98 or i==99:
                    target=np.array([1,0])
                else :
                    target=np.array([0,1])
        
                    
                if i==ACALvessels[0] and j==0:
                    feature1=pressure1
                    feature2=pressure2
                    feature3=flowrate1
                    feature4=flowrate2
    ...
    for j in range(0,N_mu):    
        for i in ACARvessels:
            ...
            name2="..\samples\CoW_"+str(j)+"_ACAR"+str(int(i))+"1.csv"
            ...
            if os.path.isfile(name2) and os.path.isfile(name1):
                ...
                if i==108 or i==109 or i==110 or i==111 or i==112 or i==116 or i==117 or i==118 or i==119 or i==120 or i==121:
                    target=np.array([1,0])
                else :
                    target=np.array([0,1])
    ...
    name='model_segment_ACA_full_'+str(train_size)+'.h5'
    model.save(name)
    ...
    name='fpr_ACA_full_'+str(train_size)+'_'
    np.save(name,fpr)
    
    name='tpr_ACA_full_'+str(train_size)+'_'
    np.save(name,tpr)
    
    name='cma_ACA_full_'+str(train_size)+'_'
    np.save(name,cma)
    
    name='f1test_ACA_full_'+str(train_size)+'_'
    np.save(name,F1_test)
    
    name='P_nor_ACA_full_'+str(train_size)+'_'
    np.save(name,A_mean)
    
    name='P_std_ACA_full_'+str(train_size)+'_'
    np.save(name,A_std)
    
    name='V_nor_ACA_full_'+str(train_size)+'_'
    np.save(name,V_mean)
    
    name='V_std_ACA_full_'+str(train_size)+'_'
    np.save(name,V_std)
    
    name='Age_nor_ACA_full_'+str(train_size)+'_'
    np.save(name,Age_mean)
    
    name='Age_std_ACA_full_'+str(train_size)+'_'
